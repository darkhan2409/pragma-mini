...
Деньги: внутренний журнал против выгруженной ленты.
    ...
    python audit/2026-09-24-gen/checks/money.py --name o1-observed --clients 64 \
...
def check_ledger(states: list) -> dict:
    ...
    for client_id, items in crossing.items():
        for item in items:
            crossing_total += 1
            if item in own.get(client_id, ()):
                mirrored += 1
            elif len(problems["P2P односторонняя проводка"]) < 10:
                problems["P2P односторонняя проводка"].append(
                    f"{client_id}: {item[0]} {item[1]} -> {item[2]} {item[3]}"
                )
