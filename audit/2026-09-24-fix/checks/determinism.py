...
Сравнение двух прогонов: где именно они совпадают и где нет.
    ...
    python audit/2026-09-24-fix/checks/determinism.py --left p0-pilot-c --right d1-repeat
...
def main() -> int:
    ...
    # Относительный --out считается от каталога аудита, а не от
    # текущего: иначе доказательство ляжет туда, откуда позвали.
    destination = (
        (AUDIT / args.out)
        if args.out
