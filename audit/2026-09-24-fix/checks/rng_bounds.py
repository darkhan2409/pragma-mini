...
Границы и ключи генератора случайности — контролируемыми входами.
    ...
    python audit/2026-09-24-fix/checks/rng_bounds.py
...
def check_integers_at_one() -> None:
    """
    Единица подаётся ДВУМЯ способами, и они означают разное.

    Fixed(1.0) переопределяет сам random(), то есть подаёт
    состояние, которого у настоящего потока больше не бывает:
    граница прижата внутри random(). Поэтому его результат
    помечен справкой, а не находкой.

    Настоящая проверка — подменить хеш на максимальный uint64 и
    пройти весь путь вычисления, как он идёт в прогоне.
    """

    value = Fixed(1.0).integers(0, 10)

    record(
        "H2 следствие: integers при переопределённом random()",
        "СПРАВКА",
        f"integers(0, 10) вернул {value}; состояние u == 1.0 недостижимо: "
        "random() прижимает границу (rng.py, _BELOW_ONE), "
        "так что это свойство самого integers, а не пути прогона",
    )

    # Тот же вопрос, но через настоящий путь: максимальный хеш.
    import hashlib
    import types
    ...
    original = rng.hashlib

    rng.hashlib = types.SimpleNamespace(blake2b=lambda *a, **k: OneDigest)

    try:
        stream = rng.KeyedRandom((1, 2, 3))
        drawn = stream.random()
        highest = rng.KeyedRandom((1, 2, 3)).integers(0, 10)
        picked = rng.KeyedRandom((1, 2, 3)).choice(["a", "b", "c"])
        certain = rng.KeyedRandom((1, 2, 3)).chance(1.0)
    finally:
        rng.hashlib = original

    ok = drawn < 1.0 and highest < 10 and picked == "c" and certain is True

    record(
        "H2 при максимальном uint64 на настоящем пути",
        "ОПРОВЕРГНУТА" if ok else "ПОДТВЕРЖДЕНА",
        f"random() = {drawn!r}, integers(0,10) = {highest}, "
        f"choice вернул {picked!r}, chance(1.0) = {certain}",
    )
...
def check_cache_key() -> None:
    ...
    try:
        ...
        # Сдвиг внутри горизонта планирования: окно за ним
        # отвергается activate_horizon, а проверяется здесь
        # ключ кэша, а не граница планирования.
        config.activate_horizon(start, end - timedelta(days=30))
