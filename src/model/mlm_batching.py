...
#   field_id   ФИЗИЧЕСКОЕ поле               → своя голова
#
...
class MaskedTargets:
    ...
    # в representations, и лишний ключ там был бы связью,
    # которой нет.
    ...
    def fields_present(self) -> tuple[int, ...]:
        return tuple(int(key) for key in np.unique(self.field_ids)) if self.n else ()
    ...
    def tensors(self, device=None) -> dict[str, torch.Tensor]:
        ...
        made = {
            ...
            "field_ids": torch.from_numpy(np.ascontiguousarray(self.field_ids)),
    ...
    def digest(self) -> str:
        """
        Отпечаток набора целей В ЭТОМ СЛОВАРЕ.

        Зависит от token id, поэтому пересборка словаря его
        меняет, даже когда задача та же. Годится для resume и
        для сверок внутри одного режима.
        """

        parts = [
            np.asarray(self.flat, dtype=np.int64),
            np.asarray(self.field_ids, dtype=np.int64),
    ...
    def field_digest(self) -> str:
        """
        Отпечаток ЗАДАЧИ, не словаря.

        Замаскированные позиции, поле каждой цели и локальный
        индекс правильного ответа. Всё это не зависит от режима
        словаря, поэтому у четырёх арок отпечаток одинаков — и
        это единственный честный способ доказать, что они решают
        одну задачу, когда token id у них разные.
        """

        parts = [
            np.asarray(self.flat, dtype=np.int64),
            np.asarray(self.field_ids, dtype=np.int64),
            np.asarray(self.local_targets, dtype=np.int64),
        ]
...
def build_targets(history: HistoryBatch, table: FieldTable) -> MaskedTargets:
    ...
    local_targets = table.to_local(field_ids, global_targets)
