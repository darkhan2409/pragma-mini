...
class Event:
    ...
    # связывает возврат с его покупкой. В выгрузку не попадает и
    # заменой event_id не является.
    #
    # Ключом случайности он больше не служит: событие,
    # датированное за концом окна, короткая выгрузка не создаёт
    # вовсе, и нумерация следующих съезжает. Дефекты наблюдения
    # ключуются по содержанию события (observe/defects.py).
    ordinal: int = 0
