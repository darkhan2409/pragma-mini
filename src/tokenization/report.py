...
def render_compatibility_md(report: dict, tokenization: dict | None = None) -> str:
    ...
    out.append(
        "\nПрежний `src/model` удалён следом: он зависел от того же токенизатора и не "
        "импортировался. Слой модели пишется заново, и список требований к нему перечислен "
        "здесь.\n"
    )
